...
Недавно поступили жалобы от пользователей, что задачи формулируются неправильно. Проблема локализована на уровне LLM-модели: необходимо доработать используемый промпт.
...
### Этап 2.
Воспроизведу базовую обработку информации из диалога, которая будет включать обработку текста диалога и генерацию задания на основе обработанного текста.
...
```bash
    ...
    dialogues. Tasks are to be formulated in the concise and easy understandable way. 
    If the task consists of multiple steps, formulate it including all steps. 
    Translate the final answer into Russian language.  Do not display not translated information. 
...
- общий тип задачи в зависимости от наполнения. Данная кластеризация будет уникальной для каждого клиента, но можно рассмотреть общие перечни кластеров в зависимость от рода деятельности компании. Например, торговые компании или из сферы услуг и т.д. Примеры типов задач: напоминания клиентам или внутри компании, работа с заказами, уточнение информации о продуктах или услугах, запись на услугу, получение информации из базы данных, внесение изменений в базу данных и т.д.
По итогам данного этапа должен быть сформирован пул задач, которые можно автоматизировать, так как они типовые и их наполнение возможно осуществить имеющимися техническими средствами (есть API), нет юридических рисков и т.д.
### Шаг 2.
На данном этапе нужно обучить классификатор, который будет относить диалог и/или задачу к определенному типу. При этом классификация скорее всего будет multi -label, так как по итогам диалога менеджера с клиентом может быть несколько типов задач. Для классификатора нужен датасет с разметкой на тип задачи, потенциально его можно получить на предыдущем шаге. Можно использовать полуавтоматическую разметку, когда первоначальную классификацию сделает генеративная модель, а после ее пересмотрит оценщики.
### Шаг 3.
Начать автоматизацию можно с очень простых задач (составить очередь), например, напоминаний. При этом желательно обеспечить дополнительный уровень проверки тех действий, который генерирует языковая модель, особенно в части информации, уходящей к клиентам. Это может быть или отдельный менеджер, или языковая модель. К сожалению, пока многие модели еще подвержены галлюцинациям.

На мой взгляд, сделать часть задач автоматизировано выполняемых LLM вполне реально, важно также правильно оценить риски и добавить необходимые mitigations.
